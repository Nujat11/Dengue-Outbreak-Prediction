...
axes[1, 0].axis('equal')
# Build legend labels that include percentage values to avoid overlapping text
total = seasonal_total.values.sum()
percentages = 100.0 * seasonal_total.values / total
legend_labels = [f"{s} — {p:.1f}%" for s, p in zip(seasonal_total.index, percentages)]
axes[1, 0].legend(wedges, legend_labels, title='Season', loc='center left', bbox_to_anchor=(1, 0.5), fontsize=11)
